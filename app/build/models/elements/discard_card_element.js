...
var DiscardCardElement = /** @class */ (function (_super) {
    ...
    function DiscardCardElement(card_count_range, then_draw) {
        ...
        if (_this.card_count_range.magnitude == 0) {
            _this.canvas_title = "Draw exactly " + _this.card_count_range.min + " card" + (_this.card_count_range.max > 1 ? "s" : "");
        }
        else {
            _this.canvas_title = "Draw " + _this.card_count_range.min + "-" + _this.card_count_range.max + " cards";
        }
        return _this;
    }
    DiscardCardElement.load = function (data) {
        return new DiscardCardElement(new number_range_1.NumberRange(data['range']), data['then_draw']);
    };
