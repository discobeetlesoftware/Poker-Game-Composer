function gameTitle(text, opt, container) {
  var title = new Label({
    text: text,
    size: opt.size,
    font: opt.family,
...
function card(isFaceDown, type, x, y) {
  var config = window.config.element.card;
  var configFace = isFaceDown ? config.faceDown : config.faceUp;
...
function cardPile(isFaceDown, count, text) {
  if (count <= 0) { return; }
  var pileConfig = window.config.element.card.pile.offset;
  var container = new Container();
  for (var j = 0; j < count; j++) {
    var pileCard = card(isFaceDown, text);
